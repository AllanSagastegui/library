...
class GetAllBooksByPublisherIdHandlerTest {

    @Mock
    private IGetAllBooksByPublisherIdUseCase useCase;
    @Mock
    ...
    private GetAllBooksByPublisherIdHandler handler;

    @BeforeEach
    void setUp() {
        handler = new GetAllBooksByPublisherIdHandler(useCase, mapper, validator, mapBookComplete);
    }

    @Test
    void listenGETAllBooksByPublisherIdUseCase() {
        int page = 0;
        int size = 10;
        ...
        when(serverRequest.pathVariable("publisherId")).thenReturn(publisherId.toString());
        ...
        when(useCase.getAllBooksByPublisherId(page, size, publisherId)).thenReturn(Mono.just(pageableDomain));
        when(mapBookComplete.toResponse(any())).thenReturn(pageableResponse);

        StepVerifier.create(handler.listenGETAllBooksByPublisherIdUseCase(serverRequest))
                .assertNext(response -> {

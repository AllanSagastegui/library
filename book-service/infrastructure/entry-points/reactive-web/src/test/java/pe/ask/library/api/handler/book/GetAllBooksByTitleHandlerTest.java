...
class GetAllBooksByTitleHandlerTest {

    @Mock
    private IGetAllBooksByTitleUseCase useCase;
    @Mock
    ...
    private GetAllBooksByTitleHandler handler;

    @BeforeEach
    void setUp() {
        handler = new GetAllBooksByTitleHandler(useCase, mapper, validator, mapBookComplete);
    }

    @Test
    void listenGETAllBooksByTitleUseCase() {
        int page = 0;
        int size = 10;
        String title = "Java";
        ...
        when(serverRequest.pathVariable("title")).thenReturn(title);
        ...
        when(useCase.getAllBooksByTitle(page, size, title)).thenReturn(Mono.just(pageableDomain));
        when(mapBookComplete.toResponse(any())).thenReturn(pageableResponse);

        StepVerifier.create(handler.listenGETAllBooksByTitleUseCase(serverRequest))
                .assertNext(response -> {

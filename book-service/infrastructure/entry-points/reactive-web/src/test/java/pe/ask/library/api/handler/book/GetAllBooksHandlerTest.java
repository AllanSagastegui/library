...
class GetAllBooksHandlerTest {
    ...
    @Mock
    private IGetAllBooksUseCase useCase;
    @Mock
    ...
    private GetAllBooksHandler handler;

    @BeforeEach
    void setUp() {
        handler = new GetAllBooksHandler(useCase, mapper, validator, mapBookComplete);
    }

    @Test
    void listenGETAllBooksUseCase() {
        int page = 0;
        int size = 10;
        ...
        when(useCase.getAllBooks(page, size)).thenReturn(Mono.just(pageableDomain));
        when(mapBookComplete.toResponse(any())).thenReturn(pageableResponse);

        StepVerifier.create(handler.listenGETAllBooksUseCase(serverRequest))
                .assertNext(response -> {

...
class GetAllBooksByLanguageUseCaseTest {
    ...
    private GetAllBooksByLanguageUseCase useCase;

    @BeforeEach
    void setUp() {
        utils = new PaginationUtils();
        useCase = new GetAllBooksByLanguageUseCase(repository, mapBookToComplete, utils);
    }

    @Test
    @DisplayName("Should return paged books by language")
    void getAllBooksByLanguage() {
        ...
        when(repository.getAllBooksByLanguage(page, size, language)).thenReturn(Flux.just(book));
        when(mapBookToComplete.map(book)).thenReturn(Mono.just(bookComplete));
        when(repository.countAllByLanguage(language)).thenReturn(Mono.just(1L));

        StepVerifier.create(useCase.getAllBooksByLanguage(page, size, language))

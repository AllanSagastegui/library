...
class GetAllBooksByCategoryIdUseCaseTest {
    ...
    private GetAllBooksByCategoryIdUseCase useCase;

    @BeforeEach
    void setUp() {
        utils = new PaginationUtils();
        useCase = new GetAllBooksByCategoryIdUseCase(repository, mapBookToComplete, utils);
    }

    @Test
    @DisplayName("Should return paged books by category ID")
    void getAllBooksByCategoryId() {
        ...
        when(repository.getAllBooksByCategoryId(page, size, categoryId)).thenReturn(Flux.just(book));
        when(mapBookToComplete.map(book)).thenReturn(Mono.just(bookComplete));
        when(repository.countAllByCategoryId(categoryId)).thenReturn(Mono.just(1L));

        StepVerifier.create(useCase.getAllBooksByCategoryId(page, size, categoryId))

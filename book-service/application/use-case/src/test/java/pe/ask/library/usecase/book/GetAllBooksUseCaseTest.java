...
class GetAllBooksUseCaseTest {
    ...
    void setUp() {
        ...
        useCase = new GetAllBooksUseCase(repository, mapBookToComplete, utils);
    }
    ...
    void getAllBooks() {
        ...
        when(repository.getAllBooks(page, size)).thenReturn(Flux.just(book));
        when(mapBookToComplete.map(book)).thenReturn(Mono.just(bookComplete));
        when(repository.countAll()).thenReturn(Mono.just(1L));

        StepVerifier.create(useCase.getAllBooks(page, size))
